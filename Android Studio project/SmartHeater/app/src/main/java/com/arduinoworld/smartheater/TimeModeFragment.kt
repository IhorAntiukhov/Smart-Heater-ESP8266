    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            ...
            buttonAddTime.setOnClickListener {
                vibrate()
                if (timestampArrayList.size < 10) {
                    timePicker.show(requireActivity().supportFragmentManager, "timePicker")
                } else {
                    Toast.makeText(requireActivity(), "Вы добавили 10\nотметок времени\nвключения/\nвыключения!", Toast.LENGTH_LONG).show()
                }
            }
                ...
                if (isNetworkConnected()) {
                    if (sharedPreferences.getInt("Temperature", -1) == -1) {
                        if (timestampArrayList.size >= 1) {
                            ...
                            realtimeDatabase.child(firebaseAuth.currentUser!!.uid).child("heaterOnOffTime").setValue(heaterOnOffTimeNodeValue)
