...
import com.arduinoworld.smartheater.databinding.FragmentTemperatureModeBinding
import com.google.firebase.database.DataSnapshot
import com.google.firebase.database.DatabaseError
import com.google.firebase.database.ValueEventListener

class TemperatureModeFragment : Fragment() {
    private lateinit var binding: FragmentTemperatureModeBinding
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding) {
            ...
            if (sharedPreferences.getInt("Temperature", -1) == -1) {
                ...
                val minTemperature = inputTemperature.text.toString().toInt() - sharedPreferences.getString("ReducingTheMinTemperature", "2").toString().toInt()
                val maxTemperature = inputTemperature.text.toString().toInt() + sharedPreferences.getString("IncreasingTheMaxTemperature", "1").toString().toInt()
                ...
                layoutProgressBar.animate().translationX(0f).alpha(1f).setDuration(500).setStartDelay(100).start()
                ...
                if (isNetworkConnected()) {
                    if (!sharedPreferences.getBoolean("TimeModeStarted", false)) {
                        if (inputTemperature.text!!.isNotEmpty()) {
                            ...
                            editPreferences.putInt("Temperature", inputTemperature.text!!.toString().toInt())
                            editPreferences.putBoolean("isHeaterStarted", false).apply()

                            val minTemperature = inputTemperature.text.toString().toInt() - sharedPreferences.getString("ReducingTheMinTemperature", "2").toString().toInt()
                            val maxTemperature = inputTemperature.text.toString().toInt() + sharedPreferences.getString("IncreasingTheMaxTemperature", "1").toString().toInt()
                            realtimeDatabase.child(firebaseAuth.currentUser!!.uid).child("temperatureRange")
                            ...
                            layoutProgressBar.translationX = 0f
            ...
            buttonStopTemperatureMode.setOnClickListener {
                ...
                if (isNetworkConnected()) {
                    ...
                    realtimeDatabase.child(firebaseAuth.currentUser!!.uid).child("temperatureRange").setValue(" ")
                    realtimeDatabase.child(firebaseAuth.currentUser!!.uid).child("isHeaterStarted")
            ...
            buttonStartHeater.setOnClickListener {
                ...
                if (isNetworkConnected()) {
                    if (!sharedPreferences.getBoolean("TimeModeStarted", false)) {
                        ...
                        realtimeDatabase.child(firebaseAuth.currentUser!!.uid).child("isHeaterStarted").setValue(true)
            ...
            buttonStopHeater.setOnClickListener {
                ...
                if (isNetworkConnected()) {
                    isAnimationStarted = false
                    isHeaterStarted = false
                    realtimeDatabase.child(firebaseAuth.currentUser!!.uid).child("isHeaterStarted").setValue(false)
                    editPreferences.putBoolean("isHeaterStarted", false).apply()
    ...
    override fun onCreateView(
        ...
        binding = FragmentTemperatureModeBinding.inflate(layoutInflater, container, false)
        return binding.root
    }
    ...
    private val temperatureListener = object: ValueEventListener {
        override fun onDataChange(snapshot: DataSnapshot) {
            ...
            binding.textTemperature.text = getString(R.string.text_temperature, snapshotValue)
